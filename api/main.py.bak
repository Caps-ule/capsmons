    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            for login in participants:
                ...
                prev_sid = (int(srow[1]) if (has_last_session_id and srow and len(srow) > 1 and srow[1] is not None) else None)

                new_count = (prev_count + 1) if (prev_sid == sid - 1) else 1

                if not has_last_session_id:
                    # Pas de colonne last_session_id -> streak non-consécutif (safe)
                    new_count = 1
